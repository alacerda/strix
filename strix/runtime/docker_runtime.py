...
class DockerRuntime(AbstractRuntime):
    ...
    def delete_scan_containers(self, scan_id: str) -> None:
        """Delete all Docker containers associated with a scan.
        
        Uses multiple search strategies to find containers:
        1. By exact name: strix-scan-{scan_id}
        2. By label: strix-scan-id={scan_id}
        ...
        for container_id, container in containers_to_remove.items():
            try:
                ...
                container_id_str = container.id or "unknown"
                
                logger.info(
                    f"Processing container for removal: {container_name_found} "
        ...
        if not containers_to_remove:
            ...
            logger.info(
                f"Container deletion summary for scan {scan_id}: "
